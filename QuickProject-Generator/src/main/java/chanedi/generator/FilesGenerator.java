...
public class FilesGenerator {
    ...
    private String inputSqlPath = "E:\\IDEA\\QuickProject\\QuickProject-Generator\\src\\test\\resources\\input.sql";
    ...
    public void parseBean() throws IOException {
        CharStream input = new ANTLRFileStream(inputSqlPath);
        ...
        for (Bean bean : beans) {
            ...
            for (TemplateRoot templateRoot : templateRoots) {
                ...
                for (String templateName : templateNames) {
                    ...
                    String destPath = parseDestPath(templateName, bean, config);
    ...
    private String parseDestPath(String templateName, Bean bean, Config config) {
        String destPath = templateName.replaceFirst("[^/]+/", config.getRootPath() + "/");
        destPath = destPath.replaceAll("\\$\\{bean\\.name\\}", bean.getName());// TODO
        int indexOfFileExtension = destPath.lastIndexOf('.');
        destPath = destPath.substring(0, indexOfFileExtension + 1);
